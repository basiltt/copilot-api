...
describe("forwardError — HTTPError with JSON body", () => {
  test("forwards Copilot JSON error object directly", async () => {
    ...
    const err = makeHTTPError(
      JSON.stringify({ error: { message: "prompt token count of 55059 exceeds the limit of 12288", code: "model_max_prompt_tokens_exceeded" } }),
      400,
    )
    ...
    expect((body as { error: { code: string } }).error.code).toBe("model_max_prompt_tokens_exceeded")
  })
  ...
  test("wraps plain-text error in envelope", async () => {
    ...
    expect((body as { error: { message: string; type: string } }).error.type).toBe("error")
    expect((body as { error: { message: string } }).error.message).toBe("Bad Gateway")
  })

  test("preserves status code from upstream", async () => {
    const jsonFn = mock()
    const c = makeContext(jsonFn)
    const err = makeHTTPError(JSON.stringify({ error: { message: "not found" } }), 404)
    await forwardError(c, err)
...
describe("forwardError — non-HTTPError", () => {
  test("returns 500 with error message", async () => {
    ...
    expect((body as { error: { message: string } }).error.message).toBe("something broke")
  })
})
